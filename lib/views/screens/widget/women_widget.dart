...
class WomenProductWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final Stream<QuerySnapshot> _productsStream =
        FirebaseFirestore.instance.collection('products').where('category', isEqualTo: 'Beer').snapshots();
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        if(snapshot.data!.docs.isEmpty){
          return Center(child: Text('Không có sản phẩm dành cho người lớn',style: TextStyle(fontSize: 18,fontWeight: FontWeight.bold, color: Colors.blueGrey)),);
        }
